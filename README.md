...
- 通过 Cordova Plugins 安装，要求 Cordova CLI 5.0+：
  ...
  ```shell
  cordova plugin add jpush-phonegap-plugin-greetgo --variable APP_KEY=your_jpush_appkey
  ```

- 或直接通过 url 安装：

  ```shell
  cordova plugin add https://github.com/flinkcstou/jpush-phonegap-plugin-greetgo.git --variable APP_KEY=your_jpush_appkey
  ```
...
### 参数
  ...
  - 同时动态配置 APP_KEY 和 CHANNEL 示例
    ...
    ```shell
    cordova plugin add jpush-phonegap-plugin-greetgo --variable APP_KEY=your_jpush_appkey --variable CHANNEL=your_channel
    ```
